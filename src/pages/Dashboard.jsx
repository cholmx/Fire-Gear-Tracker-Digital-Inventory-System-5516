...
const { 
  FiPackage, FiMapPin, FiAlertTriangle, FiCheckSquare, 
  FiPlus, FiArrowRight, FiActivity, FiTool, FiEye 
} = FiIcons

const Dashboard = () => {
  const { user } = useAuth()
  const { stations, equipment, inspections, categoryInspections, getInspectionStatus, loading, error } = useData()
    ...
    return (
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-inter-tight font-bold text-mission-text-primary">Dashboard</h1>
            <p className="text-mission-text-secondary">Loading your data...</p>
          </div>
        </div>
    ...
    return (
      <div className="space-y-6 p-6">
        <div className="bg-red-900/20 border border-red-800 rounded-lg p-6 text-center">
          <SafeIcon icon={FiAlertTriangle} className="w-12 h-12 text-red-400 mx-auto mb-4" />
          <h2 className="text-xl font-inter-tight font-bold text-red-400 mb-2">
            Dashboard Error
          </h2>
          ...
          <button
            ...
            className="bg-fire-red hover:bg-fire-red-dark text-white px-4 py-2 rounded-lg transition-colors"
          >
            Reload Dashboard
          </button>
        </div>
  ...
  const getCriticalAlerts = () => {
    ...
    // Check for overdue and critical inspections
          ...
          <div className="bg-red-950/20 border border-red-800/30 rounded-md p-4 hover:bg-red-950/30 transition-colors mission-glow-red">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className="status-dot status-critical w-2 h-2 bg-red-500 rounded-full mission-pulse" />
                <div>
        ...
        <div className="bg-mission-bg-secondary border border-mission-border rounded-md">
          <div className="px-4 py-3 border-b border-mission-border flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-base font-inter-tight font-bold text-mission-text-primary">
                Important Alerts
              </h2>
                ...
                {criticalAlerts.map((alert) => (
                  ...
                  >
                    <div className="flex items-start space-x-3">
                      ...
                      <SafeIcon icon={FiEye} className="w-4 h-4 text-mission-text-muted group-hover:text-mission-accent-blue transition-colors" />
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <div className="p-4">
            {equipment.length === 0 ? (
              <div className="text-center py-6">
                ...
                <p className="text-xs font-roboto-mono text-mission-text-muted mb-4">Get started by adding equipment</p>
                <Link
                  to="/app/equipment"
              ...
              <div className="space-y-4">
                {equipment
                  ...
                  .map((item) => (
                    <div
                      key={item.id}
                      className="flex items-center space-x-3 p-3 bg-mission-bg-tertiary hover:bg-mission-bg-primary rounded-lg transition-colors"
                    >
