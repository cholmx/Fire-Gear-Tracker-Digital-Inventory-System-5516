...
const {FiPlus,FiCalendar,FiCheckSquare,FiClock,FiAlertTriangle,FiTrash2,FiLayers,FiPrinter,FiCheck,FiEye,FiUser,FiPhone,FiMail,FiTool,FiMapPin,FiFileText}=FiIcons;

const Inspections=()=> {
const location=useLocation();
const {equipment,stations,inspections,categoryInspections,vendors,addInspection,addCategoryInspection,updateInspection,updateCategoryInspection,deleteInspection,deleteCategoryInspection,getInspectionStatus}=useData();
...
.date {font-size: 8px;}
.inspection-details {display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 12px;}
...
.equipment-list {width: 100%; border-collapse: collapse; margin-bottom: 12px;}
.equipment-list th, .equipment-list td {border: 1px solid #ddd; padding: 2px; text-align: left; vertical-align: top; font-size: 7px;}
...
.signature-section {margin-top: 12px; display: grid; grid-template-columns: 1fr 1fr; gap: 20px;}
.signature-box {border-top: 1px solid #000; padding-top: 6px; text-align: center; font-size: 7px;}
.notes-section {margin-top: 12px; page-break-inside: avoid;}
.notes-box {border: 1px solid #ddd; min-height: 40px; padding: 4px; margin-top: 4px;}
@media print {
...
.filter(inspection=> inspection.inspectionStatus?.status==='critical' || inspection.inspectionStatus?.status==='warning');
...
const getEquipmentCount=(inspection)=> {
if (inspection.type==='category') {
...
return 1; // Individual equipment inspection
}
};
...
<div>
<h1 className="text-2xl font-inter-tight font-bold text-white">Inspections</h1>
...
{inspections.length + categoryInspections.length===0 ? 'Schedule your first inspection to get started' : 'Try adjusting your filter'}
</p>
...
{inspection.vendorId && (
<div className="text-xs font-roboto-mono text-mission-accent-blue">
Vendor: {getVendorName(inspection.vendorId)}
</div>
)}
</td>
...
<div className="flex items-center space-x-2">
<SafeIcon
icon={selectedInspection.type==='category' ? FiLayers : FiCheckSquare}
className={`w-5 h-5 ${
selectedInspection.type==='category' ? 'text-mission-accent-purple' : 'text-mission-accent-blue'
}`}
/>
<span className={`text-sm font-roboto-mono ${
selectedInspection.type==='category' ? 'text-mission-accent-purple' : 'text-mission-accent-blue'
}`}>
{selectedInspection.type==='category' ? 'CATEGORY INSPECTION' : 'INDIVIDUAL INSPECTION'}
</span>
</div>
</div>
...
<div>
<h3 className="text-lg font-inter-tight font-medium text-white mb-4">Inspection Information</h3>
<div className="space-y-3">
<div>
<label className="block text-sm font-inter text-mission-text-muted">Inspection Name</label>
<p className="text-white font-inter">{selectedInspection.name}</p>
</div>
<div>
<label className="block text-sm font-inter text-mission-text-muted">Due Date</label>
<p className="text-white font-roboto-mono">
{format(new Date(selectedInspection.dueDate), 'MMMM dd, yyyy')}
</p>
</div>
{selectedInspection.lastCompleted && (
<div>
<label className="block text-sm font-inter text-mission-text-muted">Last Completed</label>
<p className="text-white font-roboto-mono">
{format(new Date(selectedInspection.lastCompleted), 'MMMM dd, yyyy')}
</p>
</div>
)}
<div>
...
<InspectionBadge inspectionStatus={selectedInspection.inspectionStatus} />
) : (
<span className="text-mission-text-muted text-sm">Unknown</span>
)}
</div>
{selectedInspection.templateId && INSPECTION_TEMPLATES[selectedInspection.templateId] && (
<div>
<label className="block text-sm font-inter text-mission-text-muted">Regulation/Standard</label>
<p className="text-white font-inter">
{INSPECTION_TEMPLATES[selectedInspection.templateId].regulation}
</p>
</div>
)}
</div>
...
<div>
<h3 className="text-lg font-inter-tight font-medium text-white mb-4">Equipment & Location</h3>
<div className="space-y-3">
{selectedInspection.type==='category' ? (
<>
<div>
<label className="block text-sm font-inter text-mission-text-muted">Category</label>
<p className="text-white font-inter">
{getCategoryName(selectedInspection.category)}
</p>
</div>
<div>
<label className="block text-sm font-inter text-mission-text-muted">Station</label>
<p className="text-white font-inter">
{selectedInspection.stationId ? getStationName(selectedInspection.stationId) : 'All Stations'}
</p>
</div>
<div>
<label className="block text-sm font-inter text-mission-text-muted">Equipment Count</label>
<p className="text-white font-roboto-mono">
{getEquipmentCount(selectedInspection)} items
</p>
</div>
</>
) : (
<>
<div>
<label className="block text-sm font-inter text-mission-text-muted">Equipment</label>
<p className="text-white font-inter">
{getEquipmentName(selectedInspection.equipmentId)}
</p>
</div>
{selectedInspection.equipmentId && (() => {
const item = equipment.find(e => e.id === selectedInspection.equipmentId);
return item && (
<div>
<label className="block text-sm font-inter text-mission-text-muted">Station</label>
<p className="text-white font-inter">
...
{/* Vendor Information */}
{selectedInspection.externalVendor && (
<div>
<h3 className="text-lg font-inter-tight font-medium text-white mb-4 flex items-center">
<SafeIcon icon={FiTool} className="w-5 h-5 mr-2 text-mission-accent-blue" />
External Vendor Required
</h3>
<div className="bg-mission-bg-tertiary rounded-lg p-4">
{selectedInspection.vendorId && (() => {
const vendor = getVendorDetails(selectedInspection.vendorId);
return vendor ? (
<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
<div>
<h4 className="text-base font-inter-tight font-medium text-white mb-3">
{vendor.name}
</h4>
<div className="space-y-2">
{vendor.contactPerson && (
<div className="flex items-center space-x-2">
<SafeIcon icon={FiUser} className="w-4 h-4 text-mission-text-muted" />
<span className="text-sm text-mission-text-secondary">{vendor.contactPerson}</span>
</div>
)}
{vendor.phone && (
<div className="flex items-center space-x-2">
<SafeIcon icon={FiPhone} className="w-4 h-4 text-mission-text-muted" />
<span className="text-sm text-mission-text-secondary">{vendor.phone}</span>
</div>
)}
{vendor.email && (
<div className="flex items-center space-x-2">
<SafeIcon icon={FiMail} className="w-4 h-4 text-mission-text-muted" />
<span className="text-sm text-mission-text-secondary">{vendor.email}</span>
</div>
)}
{vendor.address && (
<div className="flex items-start space-x-2">
<SafeIcon icon={FiMapPin} className="w-4 h-4 text-mission-text-muted mt-0.5" />
<span className="text-sm text-mission-text-secondary">{vendor.address}</span>
</div>
)}
</div>
</div>
...
{vendor.services && (
<div>
<label className="block text-sm font-inter text-mission-text-muted mb-1">Services</label>
<p className="text-sm text-mission-text-secondary">{vendor.services}</p>
</div>
)}
{vendor.certifications && (
<div className="mt-3">
<label className="block text-sm font-inter text-mission-text-muted mb-1">Certifications</label>
<p className="text-sm text-mission-text-secondary">{vendor.certifications}</p>
</div>
)}
{vendor.notes && (
<div className="mt-3">
<label className="block text-sm font-inter text-mission-text-muted mb-1">Notes</label>
<p className="text-sm text-mission-text-secondary">{vendor.notes}</p>
</div>
)}
</div>
</div>
...
{selectedInspection.vendorContact && !selectedInspection.vendorId && (
<div>
<label className="block text-sm font-inter text-mission-text-muted mb-2">Custom Vendor Contact</label>
<p className="text-white font-inter">{selectedInspection.vendorContact}</p>
</div>
)}
{!selectedInspection.vendorId && !selectedInspection.vendorContact && (
<p className="text-mission-text-muted text-sm italic">
External vendor required but no contact information provided
</p>
)}
</div>
</div>
...
<div>
<h3 className="text-lg font-inter-tight font-medium text-white mb-2 flex items-center">
<SafeIcon icon={FiFileText} className="w-5 h-5 mr-2 text-mission-accent-orange" />
Notes
</h3>
...
{selectedInspection.templateId && INSPECTION_TEMPLATES[selectedInspection.templateId] && (
<div>
<h3 className="text-lg font-inter-tight font-medium text-white mb-2">Template Information</h3>
<div className="bg-mission-bg-tertiary rounded-lg p-4">
<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
<div>
<label className="block text-sm font-inter text-mission-text-muted mb-1">Template</label>
<p className="text-white font-inter">
{INSPECTION_TEMPLATES[selectedInspection.templateId].name}
</p>
</div>
<div>
<label className="block text-sm font-inter text-mission-text-muted mb-1">Interval</label>
<p className="text-white font-inter">
{INSPECTION_TEMPLATES[selectedInspection.templateId].interval} months
</p>
</div>
<div className="md:col-span-2">
<label className="block text-sm font-inter text-mission-text-muted mb-1">Description</label>
<p className="text-mission-text-secondary font-inter text-sm">
{INSPECTION_TEMPLATES[selectedInspection.templateId].description}
</p>
</div>
</div>
</div>
...
<div className="pt-4 border-t border-mission-border">
...
vendorContact: selectedVendor ? `${selectedVendor.name}${selectedVendor.phone ? ` - ${selectedVendor.phone}` : ''}${selectedVendor.email ? ` - ${selectedVendor.email}` : ''}` : ''
});
}}
className="w-full px-3 py-2 bg-mission-bg-tertiary border border-mission-border rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-fire-red focus:border-transparent"
>
<option value="">Select a vendor or enter custom contact</option>
{vendors.map((vendor)=> (
<option key={vendor.id} value={vendor.id}>
{vendor.name} {vendor.services && `(${vendor.services})`}
</option>
))}
</select>
</div>
...
className="w-full px-3 py-2 bg-mission-bg-tertiary border border-mission-border rounded-lg text-white placeholder-mission-text-muted focus:outline-none focus:ring-2 focus:ring-fire-red focus:border-transparent"
placeholder="Vendor name and contact info"
/>
<p className="text-xs text-mission-text-muted mt-1">
Auto-filled when selecting a vendor, or enter custom contact information
</p>
</div>
</>
)}
